    ...
    for ix in 0..snx {
        ...
        for iy in 0..sny {
            ...
            for iz in 0..snz {
                ...
                for c in 0..3 {
                    mag2 += u_hat[c][[ix, iy, iz]].re.powi(2) + u_hat[c][[ix, iy, iz]].im.powi(2);
                }
...
pub fn compensated_spectrum(shells: &[f64], spectrum: &[f64]) -> Vec<f64> {
    shells
        .iter()
        .zip(spectrum.iter())
        .map(|(&k, &e)| if k > 0.0 { k.powf(5.0 / 3.0) * e } else { 0.0 })
        .collect()
}
...
mod tests {
    ...
    fn spectrum_single_mode() {
        ...
        u_hat[0][[1, 0, 0]] = Complex {
            re: 0.0,
            im: -0.5 * (n * n * n) as f64,
        };
        u_hat[0][[n - 1, 0, 0]] = Complex {
            re: 0.0,
            im: 0.5 * (n * n * n) as f64,
        };
        ...
        assert!(
            spec[0].abs() < 1e-10,
            "shell 0 should be ~0, got {}",
            spec[0]
        );
        for i in 2..shells.len() {
            assert!(
                spec[i].abs() < 1e-10,
                "shell {i} should be ~0, got {}",
                spec[i]
            );
        }
    }
