...
/// penalisation constants (`eta_p`, `sigma_max`). Anything else a driver
/// derives from these (body centre, radius, fringe extent) is a
/// deterministic function of them, so agreement here is enough to guarantee
/// a rebuilt body matches too.
...
/// Fixed header length in bytes: magic, version, `step`, `frame`, then the
/// nine [`CheckpointParams`] fields (`nx`, `ny` as `u64`, the rest as `f64`).
const CHECKPOINT_HEADER_LEN: usize = 8 + 4 + 8 + 8 + 8 + 8 + 8 * 7;
...
/// atomically.
///
/// Layout: the [`CHECKPOINT_HEADER_LEN`]-byte header above, followed by
/// `wh`, `gh`, `rh` in that order, each as `nx * (ny / 2 + 1)` complex
/// values in the array's own row-major iteration order, each complex value
/// as two little-endian `f64` (real, then imaginary). Every integer field is
/// little-endian too.
///
...
/// step back.
///
/// **Atomic.** Serialises to a temporary file in `dir` (so the rename below
/// stays on one filesystem) and `rename`s it onto `checkpoint.bin`. POSIX
/// `rename(2)` within one filesystem replaces the destination in a single
...
/// checkpoint (or none) rather than a truncated one. A crash mid-write does
/// leave the temporary file behind; a later run's `read_checkpoint` never
/// looks at it, since it names the target, not the temporary pattern.
pub fn write_checkpoint(
    ...
    let mut buf = Vec::with_capacity(CHECKPOINT_HEADER_LEN + 3 * params.nx * nyh * 16);
...
/// `dir/checkpoint.bin`.
///
/// Rejects the file outright on a bad magic, an unknown version, or a
/// length that disagrees with what the header's own `nx`/`ny` implies
/// (which is what a truncated write looks like), rather than reading past
/// the end or silently zero-filling the gap. Does not check the recorded
/// parameters against a config to resume into: call
...
pub fn read_checkpoint(dir: &Path) -> std::io::Result<Checkpoint> {
    ...
    let ly = f64_at(52);
    let re = f64_at(60);
    let u_mean = f64_at(68);
    let dt = f64_at(76);
    let eta_p = f64_at(84);
    let sigma_max = f64_at(92);

    let nyh = ny / 2 + 1;
    let expected_len = CHECKPOINT_HEADER_LEN + 3 * nx * nyh * 16;
